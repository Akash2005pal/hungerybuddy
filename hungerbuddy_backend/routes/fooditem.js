...
router.post('/submit_fooditem',upload.single('picture'),function(req,res){
  ...
  try{
        ...
        pool.query('insert into fooditems( branchid, categoryid, fooditemname, fooditemtype, fooditemtaste, ingridients, fullprice, halfprice, offerprice, status,ratings,picture,createddate,createdtime) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            [
                req.body.branchid,
                req.body.categoryid,
...
router.post('/edit_picture',upload.single('picture'), function(req, res, next) {
  ...
  try{
    pool.query('update fooditems set picture=? where fooditemid=?',[req.file.filename,req.body.fooditemid],function(error,result){
     if(error)
     {  console.log(error)
        res.status(500).json({status:false,message:'Database Error Please Contact Bankend Team....'})
     }
