...
router.post('/fetch_cities', function (req, res, next) {
    pool.query("select * from citys where stateid=?", [req.body.stateid], function (error, result) {
        if (error) {
            res.status(500).json({ status: false, message: 'Database Error Pls contact in backend...' })
        }
        else {
...
router.get('/fetch_all_delivery', function (req, res, next) {
    ...
    try {
        pool.query('SELECT D.*,(SELECT B.branchname FROM branch B WHERE B.branchid = D.branchid) AS branchname,(SELECT ST.statename FROM states ST WHERE ST.stateid = D.state)AS statename,(SELECT C.cityname FROM citys C WHERE C.cityid = D.city) AS cityname FROM deliverybody D', function (error, result) {
            if (error) {
                console.log(error);
                res.status(500).json({ status: false, message: 'Database Error....' });
            }
