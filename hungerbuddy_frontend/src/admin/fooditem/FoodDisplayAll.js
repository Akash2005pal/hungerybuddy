...
export default function FoodDisplayAll({ refresh, setRefresh }) {
  ...
  const validation = () => {
    ...
    if (fooditemName.length == 0) {
      setError((prev) => ({
        ...prev,
        FoodItemName: "food item Name is required",
      }));
      isError = true;
    }
    if (category.length == 0) {
      setError((prev) => ({
        ...prev,
        Category: "food item Category is required",
      }));
      isError = true;
    }
    if (fooditemType.length == 0) {
      setError((prev) => ({
        ...prev,
        FoodItemType: "food item Type is required",
      }));
      isError = true;
    }
    if (fooditemTaste.length == 0) {
      setError((prev) => ({
        ...prev,
        FoodItemTaste: "food item Taste is required",
      }));
      isError = true;
    }
    ...
    if (ingredients.length == 0) {
      setError((prev) => ({
        ...prev,
        Ingredients: "food item Ingredients is required",
      }));
      isError = true;
    }
    if (fullPrice.length == 0) {
      setError((prev) => ({
        ...prev,
        FullPrice: "food item Full Price is required",
      }));
      isError = true;
    }
    if (halfPrice.length == 0) {
      setError((prev) => ({
        ...prev,
        HalfPrice: "food item Half Price is required",
      }));
      isError = true;
    }
    if (offerPrice.length == 0) {
      setError((prev) => ({
        ...prev,
        OfferPrice: "food item Offer Price is required",
      }));
      isError = true;
    }
    if (status.length == 0) {
  ...
  const handleDelete = async (fid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        var response = await postData("fooditem/delete_fooditem", {
          fooditemid: fid,
        });
      ...
      <MaterialTable
        ...
        columns={[
          { title: "Category", field: "categoryname" },
          {
            ...
            render: (rowData) => (
              <div>
                {rowData.fooditemname}({rowData.fooditemtype})
              </div>
            ),
          },

          {
            ...
            render: (rowData) => (
              <div>
                &#8377;{rowData.fullprice}/&#8377;{rowData.halfprice}
              </div>
            ),
          },
          {
  ...
  return (
    <div className={classes.root}>
      <div className={classes.box}>{displayFoodItems()}</div>
      {showDialog()}
    </div>
  );
}
